...
// GET: Fetch student list from database
export async function GET(req: Request) {
  try {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS students_store (
    ...
    const { rows } = await sql`
      SELECT data FROM students_store WHERE id = 1
    `;
...
// POST: Save student list to database
export async function POST(req: Request) {
  try {
    ...
    await sql`
      INSERT INTO students_store (id, data, updated_at)
